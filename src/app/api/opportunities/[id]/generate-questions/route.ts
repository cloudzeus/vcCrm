...
export async function POST(
  ...
  try {
    ...
    // Get the CRM record (opportunity)
    const crmRecord = await prisma.crmRecord.findFirst({
      where: {
        id,
        organizationId: organization.id,
        status: "OPPORTUNITY", // Only generate questions for opportunities
      },
      include: {
        company: {
          include: {
...
Return ONLY the JSON array, no additional text.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a sales consultant expert at creating detailed questions to clarify customer requirements for proposals. All questions must be in GREEK LANGUAGE. Always respond with valid JSON arrays only.",
        },
        {
          role: "user",
          content: prompt,
    ...
    // Create tasks from generated questions
    ...
    await prisma.crmRecord.update({
      where: { id },
      data: {
        briefStatus: "QUESTIONS_GENERATED",
      },
    });
